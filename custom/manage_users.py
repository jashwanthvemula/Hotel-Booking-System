...
def show_user_details(event=None):
    ...
    if 'recent_bookings' in selected_user and selected_user['recent_bookings']:
        for booking in selected_user['recent_bookings']:
            # Format dates
            check_in = booking['Check_IN_Date'].strftime('%Y-%m-%d') if hasattr(booking['Check_IN_Date'], 'strftime') else booking['Check_IN_Date']
            check_out = booking['Check_Out_Date'].strftime('%Y-%m-%d') if hasattr(booking['Check_Out_Date'], 'strftime') else booking['Check_Out_Date']
...
# ----------------- Main Function -----------------
def main():
    global current_admin, first_name_entry, last_name_entry, email_entry, phone_entry
    global address_entry, password_entry, user_table, details_frame, details_user_id
    global details_name, details_email, details_phone, details_address, details_bookings
    global details_spent, bookings_table, user_count_label, create_btn, update_btn
    global delete_btn, search_entry
